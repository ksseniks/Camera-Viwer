...
class VideoHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        ready_path = f"{event.src_path}.ready"
        asyncio.run_coroutine_threadsafe(self.handle_video(event.src_path, ready_path), self.loop)

    async def handle_video(self, video_path, ready_path):
        try:
            ...
            for chat_id in SUBSCRIBED_USERS:
                try:
                    await self.application.bot.send_video(chat_id, video_path, 120)
